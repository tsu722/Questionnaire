...
<body>
  ...
  <fieldset class="layui-elem-field">
	...
	<form class="layui-form layui-form-pane" action="">
		  ...
		  <div style="margin-left:1cm;" class="layui-form-item">
		    ...
		    <div class="layui-input-inline">
		      <input type="text" name="watSrName" id="watSrName" placeholder="请输入" autocomplete="off" class="layui-input" lay-verify="required|title">
		    </div>
		  </div>
		  ...
		  <div style="margin-left:1cm;" class="layui-form-item">
		    ...
		    <div class="layui-input-inline">
		      	<select name="watSrType" id="watSrType" lay-search="" lay-verify="required">
		  ...
		  <div style="margin-left:1cm;" class="layui-form-item">
		    ...
		    <div class="layui-input-inline">
		      <input type="text" name="watQuantity" id="watQuantity" placeholder="升" autocomplete="off" class="layui-input" lay-verify="watQt">
		    </div>
		  </div>
	...
	var watLocID = -1;		//记录水源点的位置记录ID
	var watID = -1;
	
	$(document).ready(function(){
		 var tgID = parent.sendTgID();
	     var req = {"watSrID":tgID};
	     $.ajax( {
			     ...
			     success : function(data) {
			       	  if(data.msg == 1){
			       		$("#watSrName").val(data.priWat.watSrName);
			       		$("#watSrType").val(data.priWat.watSrType);
			       		$("#watQuantity").val(data.priWat.watQuantity);
			       		$("#rtID").val(data.priWat.rtID);
			       		$("#positionX").val(data.priWat.position.pointX);
			       		$("#positionY").val(data.priWat.position.pointY);
			       		watLocID = data.priWat.position.pointID;
			       		watID = tgID;
			       	  }				       		 							       	  						 								       	 
			     }				    	 				       							     
		 });
	...
	layui.use(['form', 'layedit', 'laydate'], function(){			//必须监听表单，否则某些样式出不来
	  ...
	  form.on('submit(demo1)', function(data){
	    ...
	    if(data.field.locType == "2"){
			req = {"watSrID":watID, "watSrName":data.field.watSrName, "watSrType":data.field.watSrType, 
		    		"watQuantity":data.field.watQuantity, "rtID":data.field.rtID,  
		    		"position":{"pointID":watLocID, "pointX":data.field.positionX, "pointY":data.field.positionY, "zmLev":1, "rtID": data.field.rtID, "ptType":3, "sptOrdInRt":watID}}
	    }
	    if(data.field.locType == "1"){
			req = {"watSrID":watID, "watSrName":data.field.watSrName, "watSrType":data.field.watSrType, 
		    		"watQuantity":data.field.watQuantity, "rtID":data.field.rtID,
		    		"gpsLat":data.field.positionX, "gpsLon":data.field.positionY,
		    		"position":{"pointID":watLocID, "zmLev":1, "rtID": data.field.rtID, "ptType":3, "sptOrdInRt":watID}}
	    }
