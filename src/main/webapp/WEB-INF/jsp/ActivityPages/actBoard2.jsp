...
<html>
<head>
<!-- 视频模块浏览查询图片主页面 正式网页 单页调试访问地址：/act/toActBoard2 -->
			 ...
			 $.ajax( {
				 ...
				 success : function(data) {       //success函数主要将Json数据转化为带有规范样式的节点
			 		...
			 		layui.use(['laypage', 'layer'], function(){
			 			  ...
			 			  //模拟渲染
			 			  var render = function(actBoardBody, curr){
			 			    var arr = []
			 			    ,thisData = actBoardBody.concat().splice(curr*nums-nums, nums);
			 			  ...
			 			  laypage({
			 			    cont: 'demo8'
			 			    ,pages: Math.ceil(actBoardBody.length/nums)
			 			    ,skin: '#1E9FFF'
			 			    ,jump: function(obj){
			 			      document.getElementById('biuuu_city_list').innerHTML = render(actBoardBody, obj.curr);
			 			    }
			 			  });
